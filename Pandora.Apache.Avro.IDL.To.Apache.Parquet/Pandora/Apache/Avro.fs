...
module Avro =
  ...
  module Schema =
    ...
    let rec toParquetSchema ons (env:env) (ast:ast) (t:JToken) =
      match t.Type with
        | JTokenType.Object when JSON.memberIsArray  "type" (t :?> JObject) ->
          ...
          let (env', ast', errs) =
            ...
            |> Seq.fold (
             fun (e, a, es) x ->
               let (e', a', es') = toParquetSchema ons' e a x
               ( e'
               , a'
               , seq {
                   yield! es
          ...
          if (function | Result.Error _ -> true | _ -> false) rft then
            ...
            match rft with
              ...
              | Result.Ok ((Parquet.Schema.Ast.Type.ERROR   fqdn)    as ft)  ->
                ...
                let (xs, ys, zs) = common ons' env ast o
                ( xs
                , ys
                , seq {
                    yield! zs
        ...
        | otherwise ->
          ...
          , seq {
              yield
                //( sprintf "toParquetSchema > otherwise: %s" (t.ToString())
                ( sprintf "toParquetSchema > otherwise: %A" otherwise
                )
            }
          )
    
    and private common ons (env:env) (ast:ast) (o:JObject) =
      let ons' =
        match JSON.memberStrValue "namespace" o with
          | Some ns -> Some ns
          | None    -> ons
      match JSON.memberStrValue "name" o with
        | Some uid ->
          ...
          let (env', ast', errs) =
            ...
            |> Seq.fold (
             fun (e, a, es) o ->
               let (e', a', es') = field fqdn ons' env a (o :?> JObject)
               ( e'
               , a'
               , seq {
                   yield! es
        ...
        | None     ->
          ...
          , seq {
              yield
                ( sprintf "common > missing uid"
                )
            }
          )
    
    and private field pid ons (env:env) (ast:ast) (t:JToken) =
      ...
      match JSON.memberStrValue "name" o', rft' with
        | Some uid, Result.Ok    ft ->
          ...
          if isReference o' ft then
            ...
            let (xs, ys, zs) = toParquetSchema ons' env ast o' // t
            ( xs
            , ys
            , seq {
                yield! zs
